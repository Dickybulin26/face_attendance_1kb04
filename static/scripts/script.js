...
function initRegistrationPage() {
	...
	window.submitData = async function () {
		...
		if (!imgBase64 || imgBase64.includes("window.location")) return alert("Foto tidak ada!");
...
function initHistoryPage() {
	...
	if (typeof FullCalendar !== "undefined") {
		...
		window.calendarInstance = new FullCalendar.Calendar(calendarEl, {
			...
			eventDidMount: info => {
				...
				if (dayCell) {
					...
					dayCell.addEventListener("mouseenter", () => showTooltip(eventProps, dayCell));
		...
		function showTooltip(props, targetEl) {
			...
			if (nameEl) nameEl.innerText = props.is_admin ? "Rekap Harian" : props.nama;
